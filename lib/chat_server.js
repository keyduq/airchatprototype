...
function ingresarSala(socket, sala){
    ...
    var usuariosEnSala = io.sockets.clients(sala);
    // Si otros existen resume el total de usuarios
    if (usuariosEnSala.length > 1) {
        var usuariosEnSalaSummary = 'Usuarios actualmente en ' + sala + ': ';
        for (var index in usuariosEnSala) {
            console.log(usuariosEnSala[sala]);
            var usuarioSocketId = usuariosEnSala[index].id;
...
function manejoBroadcastMensaje(socket){
    socket.on('mensaje', function (mensaje) {
        socket.broadcast.to(mensaje.sala).emit('mensaje', {
            text: nickNames[socket.id] + ': ' + mensaje.text
        });
    });
}
// Manejo de ingreso a las salas
function manejoIngresoSala(socket) {
    socket.on('join', function(sala) {
        socket.leave(actualSala[socket.id]);
        ingresarSala(socket, sala.nuevaSala);
    });
}
// Manejo de la desconexión del usuario
function manejoDesconexionCliente(socket) {
    socket.on('disconnect', function (){
        var nombreIndex = nombreUsados.indexOf(nickNames[socket.id]);
        delete nombreUsados[nombreIndex];
